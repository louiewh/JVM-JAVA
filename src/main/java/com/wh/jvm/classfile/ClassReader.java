...
import com.wh.jvm.classfile.accessflags.ClassAccessFlags;
...
public class ClassReader {
    ...
    private static ConstantPoolInfo newConstantPoolInfo(U1 tag, InputStream inputStream) {
        ...
        switch (tagValue) {
            ...
            case ConstantPoolInfo.CONSTANT_FIELD_REF_INFO:
                constantPoolInfo = new ConstantFieldRefInfo(ConstantPoolInfo.CONSTANT_FIELD_REF_INFO);
                constantPoolInfo.read(inputStream);
                break;
            case ConstantPoolInfo.CONSTANT_METHOD_REF_INFO:
                constantPoolInfo = new ConstantMethodRefInfo(ConstantPoolInfo.CONSTANT_METHOD_REF_INFO);
                constantPoolInfo.read(inputStream);
                break;
            case ConstantPoolInfo.CONSTANT_INTERFACE_METHOD_REF_INFO:
                constantPoolInfo = new ConstantInterfaceMethodRefInfo(ConstantPoolInfo.CONSTANT_INTERFACE_METHOD_REF_INFO);
                constantPoolInfo.read(inputStream);
                break;
            case ConstantPoolInfo.CONSTANT_NAME_AND_TYPE_INFO:
                constantPoolInfo = new ConstantNameAndTypeInfo(ConstantPoolInfo.CONSTANT_NAME_AND_TYPE_INFO);
                constantPoolInfo.read(inputStream);
                break;
            case ConstantPoolInfo.CONSTANT_METHOD_HANDLE_INFO:
                constantPoolInfo = new ConstantMethodHandleInfo(ConstantPoolInfo.CONSTANT_METHOD_HANDLE_INFO);
                constantPoolInfo.read(inputStream);
                break;
            case ConstantPoolInfo.CONSTANT_METHOD_TYPE_INFO:
                constantPoolInfo = new ConstantMethodTypeInfo(ConstantPoolInfo.CONSTANT_METHOD_TYPE_INFO);
                constantPoolInfo.read(inputStream);
                break;
            case ConstantPoolInfo.CONSTANT_INVOKE_DYNAMIC_INFO:
                constantPoolInfo = new ConstantInvokeDynamicInfo(ConstantPoolInfo.CONSTANT_INVOKE_DYNAMIC_INFO);
                constantPoolInfo.read(inputStream);
                break;
        }
    ...
    private static String getConstantClassInfoValue(ConstantPool constantPool, short constantClassInfoIndex) {
        if(constantClassInfoIndex <= 0 ||constantClassInfoIndex >= constantPool.getCpInfo().length){
            return "00";
        }
